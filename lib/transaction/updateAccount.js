...
const processFollowings = (tx, value, resolve, reject) => {
    ...
    Account.findOne({address: tx.account}, {followings: 1, name: 1, address: 1})
    .then(res => {
    ...
    .then(() => {
        ...
        return Account.find({address: {"$in": listUnfollowAddress}}, {followers: 1, address: 1})
    })
    ...
    .then(async res => {
        ...
        for(let i = 0; i < res.length; i++) {
            ...
            // socket unfolow
            socket.unFollowings(agent, res[i]);
        }
    ...
    .then(() => {
        ...
        return Account.find({address: {"$in": listFollowAddress}}, {followers: 1, address: 1})
    })
